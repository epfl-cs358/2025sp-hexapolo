...
def test_imports():
    ...
    try:
        ...
        logger.error(f"✗ Failed to import audio: {e}")
        return False
...
def test_audio_functionality():
    """Test audio (Direction of Arrival) functionality"""
    ...
    try:
        ...
        if doa_result is None:
            logger.info("✓ audio returned None (no sound detected)")
        elif doa_result == -999:
            logger.info("✓ audio returned -1 (stop signal)")
        else:
            logger.info(f"✓ audio detected angle: {doa_result}°")
